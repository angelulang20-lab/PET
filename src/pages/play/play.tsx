...
const PHRASES = [
    ...
    {
        status: "hungry",
        phrases: ["I'm hungry!", "Feed me please!", "My tummy is rumbling", "Need food!"]
    },
    {
        status: "depressed",
        phrases: ["I'm sad...", "Don't feel good", "Leave me alone", "Not in the mood"]
    },
    {
        status: "bloated",
    ...
    {
        status: "great",
        phrases: ["I feel amazing!", "Best day ever!", "So happy!", "Life is great!"]
    }
]
...
const BUTTON_PHRASES = [
    {
        ...
        phrases: [
            {
                liked: ["That was fun!", "Play again!", "I love playing!", "More play time!"]
            },
            {
                hated: ["I don't like playing", "Can we stop?", "Not in the mood to play", "I'm tired of playing"]
            }
        ]
    },
    {
        ...
        phrases: [
            ...
            {
                hated: ["I don't like this", "Yucky!", "Can I have something else?", "This tastes bad"]
            }
        ]
    },
    {
...
function Play(){
    ...
    const selectedTypeData = pet ? PET_TYPES.find(p => p.type === pet.petType) : null
    ...
    const buttonStyle = "w-full rounded-none bg-cyan-200 p-2 border-2 border-cyan-300 outline outline-1 outline-cyan-200 text-cyan-700 shadow-[2px_2px_0_0_#22d3ee] hover:shadow-[1px_1px_0_0_#22d3ee] hover:translate-x-[1px] hover:translate-y-[1px] transition-all active:shadow-none active:translate-x-[2px] active:translate-y-[2px]"

    const sectionStyle = "bg-cyan-50/90 p-4 flex flex-row justify-between gap-3 border-double border-8 border-cyan-300 outline outline-2 outline-cyan-200 shadow-[4px_4px_0_0_#22d3ee]"
    ...
    const handleTalkClick = () => {
        const statusPhrases = PHRASES.find(e => e.status === status)?.phrases as string[] | undefined
    ...
    const handlePlayClick = () => {
        ...
        const likesPlaying = selectedTypeData?.likes.some(like => like === 'play')
        const dislikesPlaying = selectedTypeData?.dislikes.some(dislike => dislike === 'play')
        
        const playPhrases = BUTTON_PHRASES.find(e => e.activity === 'play')?.phrases as any
    ...
    const handleFoodSelect = (foodType: string) => {
        ...
        const eatPhrases = BUTTON_PHRASES.find(e => e.activity === 'eat')?.phrases as any
    ...
    useEffect(() => {
        if ((energyValue <= 0) || (hungerValue <= 0) || (happyValue <= 0)){
            ...
            toast.warning("NOOO! YOUR PET DIED!💀 Returning to home...", {position: "bottom-center"})
            navigate('/')
        }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 py-8">
            <div className="container mx-auto px-4 max-w-2xl">
                <HeaderOne cl='font-bold text-center' text={`${pet?.petName} the ${pet?.petType}`}/>
                            ...
                            <div className="flex flex-col">
                                <div className="w-[150px] h-[150px] p-2 border-double border-4 border-cyan-300 bg-white/70 outline outline-1 outline-cyan-200 shadow-[2px_2px_0_0_#22d3ee] flex items-center justify-center">
                                    {selectedTypeData?.image && (
                                        <img src={selectedTypeData?.image} alt={selectedTypeData.displayName} className="max-w-full max-h-full object-contain rounded-lg" />
                                    )}
                                </div>
                            </div>
                            {/* talk */}
                                                ...
                                                .map((dislike) => {
                                                    ...
                                                    return (
                                                        <p key={dislike} className="flex items-center gap-2">
                                                            {foodItem?.image} {foodItem?.name || dislike}
                                                        </p>
                                                    );
                                                })}
                                            </div>
